...
const get_semester=async(req,res)=>{
    try{
        ...
        console.log("QUERY",req.query);
            var stu;
            var fl_st= Semester.aggregate().lookup({
                from: 'departments',
                localField: 'department',
                foreignField: '_id',
                as: 'department_details',
            }).unwind("$department_details")
            req.query.item||id?stu=await fl_st.match({$or:[
        {sem:parseInt(items)},
              {_id:new mongoose.Types.ObjectId(id)},{'department_details.name':{$regex:items,$options: "si"}}]})
