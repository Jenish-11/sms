...
module.exports.get_student_by_id=async(req,res)=>{
    try {
        const student = await Students.aggregate().match({_id:new mongoose.Types.ObjectId(req.body.id)}).addFields({
            photo:{$concat:[process.env.SERVER,'/',process.env.FOLDER,'/','$photo']}
        }).exec()
...
module.exports.update_students=async(req,res)=>{
    ...
    const st =  Students.findByIdAndUpdate(id,{$set:update1})
        .then(async()=> Students.findById(id)
...
module.exports.delete_students=async(req,res)=>{
    ...
    try{
        const st = await Students.deleteOne({_id:new mongoose.Types.ObjectId(req.body.id)}).exec()
...
module.exports.student_image=async(req,res)=>{
    ...
    console.log(file)
    res.json({image:`${process.env.SERVER}/${process.env.FOLDER}/${file.filename}`})
    // res.redirect(`${process.env.SERVER}/upload/${file.filename}`)
    
}
module.exports.get_image=(req,res)=>{
    // console.log(req.body)
    res.json({image:`${process.env.SERVER}/upload/${'1677756623832-DSC_0301.JPG'}`})
...
module.exports.student_login=async(req,res)=>{
    ...
    const login = St_login.create({email:req.body.email,user_agent:req.headers['user-agent'],ip:req.socket.remoteAddress,status:false})
